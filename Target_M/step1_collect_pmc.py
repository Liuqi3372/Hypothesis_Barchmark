...
def record(paper, category: str, decision: str, code: str = "") -> dict:
    """Convert one parsed PMC paper into the shared Step 1 output record."""
    return {
...
def allocate_year_quotas(start_year: int, end_year: int, count: int) -> dict[int, int]:
    """Split the final eligible-paper target evenly across publication years."""
...
def screen_paper(paper) -> tuple[dict, bool]:
    """Apply the common article-type, OA, metadata, and full-text rules."""
...
def collect_eligible_year(
    ...
    """Compatibility helper for the rule-only collector and its sampling tests."""
    if quota == 0:
    ...
    while True:
        sampling: dict = {}
        min_per_category = min(200, max(1, candidate_count // (2 * len(CATEGORIES))))
        papers = collect_fn(
            client, year, year, candidate_count,
            min_per_category=min_per_category, sampling_report=sampling,
        )
        ...
        if len(papers) <= previous_fetched or len(papers) < candidate_count:
            raise RuntimeError(
                ...
                f"but only {len(eligible)} passed the available candidates."
            )
