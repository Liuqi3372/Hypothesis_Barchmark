...
- `Target_M/step1_collect_pmc.py`：Target M 第一步的唯一入口。它从 PubMed Central 按年份收集论文，执行基础规则过滤、主题分类和均衡采样；随后审计目标论文引言中的参考文献是否可以开放获取，并根据 OA 覆盖率保留候选论文。该实现与上图中的 OA-Audited Candidate Pool 一致。
...
3. 使用同一脚本中的基础规则函数排除不合格论文，并将论文分配到研究主题类别。
4. 审计引言中去重后的参考文献，通过 PMCID 或 Unpaywall 判断能否合法开放获取；默认要求 OA 覆盖率不低于 90%、PMC 覆盖率不低于 70%、引用解析率不低于 95%。
